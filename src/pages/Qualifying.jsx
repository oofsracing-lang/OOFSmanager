...
const Qualifying = () => {
    const { qualifyingSettings, submitQualifyingResult, currentSeasonId, seasonConfig, championshipData } = useChampionship();
    const isSprint = Boolean(
        currentSeasonId?.includes('sprint') ||
        seasonConfig?.seriesName?.toLowerCase().includes('sprint') ||
        championshipData?.season?.toLowerCase().includes('sprint') ||
        seasonConfig?.classes?.includes('LMP3')
    );
    ...
    return (
        <div style={{ maxWidth: '900px', margin: '0 auto', padding: '4rem 2rem', textAlign: 'center' }}>
            ...
            <div className="glass-panel" style={{ marginTop: '2.5rem', padding: '2.5rem', textAlign: 'left' }}>
                <h2 style={{ fontSize: '1.4rem', margin: '0 0 1.25rem 0', color: 'var(--text-main)', borderBottom: '1px solid var(--border-color)', paddingBottom: '0.75rem' }}>
                    Qualifying Instructions {isSprint ? '(Sprint)' : '(Endurance)'}
                </h2>
